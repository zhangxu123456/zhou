...
class GoodsController extends CommonController {
	...
	public function goodsdetail(){    	
		...
		$goods = M('Goods');
      	$goods_description = M('Goods_description');
      	$filegoods = $goods->select();
      	foreach ($filegoods as $k1 => $v1) {
          	$proWhere['goods_id'] = $v1['goods_id'];
          	$crr=$goods_description->where($proWhere)->select();
          	$filegoods[$k1]['lv'] = $crr;
      	}
      	$this->assign('filegoods',$filegoods);
 ...
 //        $count=M('goods')->where(array('status'=>1))->count();
	...
	// public function goodsdetail(){    	
	// 	$id=get_url_id('id');
	// 	$sql="select * from ".C('DB_PREFIX')."goods where goods_id=".$id;
	// 	$goods=M()->query($sql);
