...
use Think\Model;
class PrincipleModel extends Model{
	
	//修改时，取得博客图片
	public function get_data($id){
		$d=M('Principle')->find($id);
	...
	public function get_image_data($id){
		$d=M('principle_image')->where(array('mid'=>$id))->select();
		foreach ($d as $k => $v) {
	...
	public function show_page(){
		...
		$sql='SELECT * FROM '.C('DB_PREFIX').'principle';
	...
	public function edit_page($data){
		...
		$r=M('Principle')->save($data);

		if($r){				
			try{
				M('principle_image')->where(array('mid'=>$page_id))->delete();	
				if (isset($data['blog_images'])) {						
					foreach ($data['blog_images'] as $blog_image) {
						$this->execute("INSERT INTO " . C('DB_PREFIX') . "principle_image SET mid = '" . (int)$page_id . "', image = '" . $blog_image['image']. "', title = '" . $blog_image['title'] . "', sort_order = '" . (int)$blog_image['sort_order'] . "'");
					}
				}
	...
	public function del_page($id){
		try{
			$image=M('Principle')->where(array('page_id'=>$id))->field('image')->find();			
			if(!empty($image)){
				A('Image')->del_image('blog',$image['image'],'blog');
			}

			$gallery=M('principle_image')->where(array('mid'=>$id))->field('image')->select();
			if(!empty($gallery)){
			...
			M('Principle')->where(array('page_id'=>$id))->delete();
			// M('news_content')->where(array('news_id'=>$id))->delete();
			M('principle_image')->where(array('mid'=>$id))->delete();
