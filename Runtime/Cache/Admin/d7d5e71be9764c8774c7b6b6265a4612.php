...
<html lang="en">
	...
	<span class="btn btn-primary">支付方式</span>
	</div>	
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>						
							...
							<td>
								<?php switch($data["payment_state"]): case "1": ?><span class="green bold">启用</span><?php break;?>    
									<?php case "0": ?><span class="red bold">禁用</span><?php break; endswitch;?>
							</td>
							<td>
								<a class="btn btn-xs btn-info" href='<?php echo U("Payment/edit",array("code"=>$data["payment_code"]));?>' >
